...
public class MainBase64 extends ActivityGroup implements OnClickListener, TabContentFactory, RadioGroup.OnCheckedChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(Base64Coding.analyzeAvailable())
        	analyze_button.setEnabled(true);
        if(Base64Coding.bruteAvailable())
	...
	public void onClick(View v) {
			...
			case R.id.algos_button_about:
				int text = R.string.mainbase64_about_text;
				int title = R.string.mainbase64_about_title;
				Utils.showAbout(this,title,text);
				break;
				
			case R.id.main_basic_button_go:
				try {
					if(radio_cipher.isChecked()) {//we do here the convertion
						if(!(plain_edit_text.getText().toString().equals("")))
							cipher_edit_text.setText(Base64Coding.encodeBase64(plain_edit_text.getText().toString()));
					}	
					else if(radio_decipher.isChecked()) {
						if(!(cipher_edit_text.getText().toString().equals("")))
							plain_edit_text.setText(Base64Coding.decodeBase64(cipher_edit_text.getText().toString()));
					}
				}
				catch(IllegalArgumentException e) {
					Toast.makeText(this, "Bad Base64", Toast.LENGTH_SHORT).show();
				}
				catch (Exception e) {
